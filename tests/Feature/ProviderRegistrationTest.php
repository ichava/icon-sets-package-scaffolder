...
it('publishes under vendor-namespaced tags', function (): void {
    ...
    expect(array_keys(ServiceProvider::$publishGroups))
        ->toContain('ichava::icon-package-scaffolder-config')
        ->toContain('ichava::icon-package-scaffolder-stubs')
        ->not->toContain('stubs')
        ->not->toContain('config')
        ->not->toContain('icon-package-scaffolder');
});
...
it('merges config at the key the code reads', function (): void {
    ...
    expect(config('ichava.icon-package-scaffolder'))->toBeArray()
        ->and(config()->has('ichava.icon-package-scaffolder.stubs_path'))->toBeTrue()
        ->and(config('ichava.icon-package-scaffolder.icon-package-scaffolder'))->toBeNull();
});
...
it('honours a configured stubs path', function (): void {
    ...
    config()->set('ichava.icon-package-scaffolder.stubs_path', $custom);
