...
it('produces the values core produced, so an extracted pack is unchanged', function (): void {
    ...
    expect($tokens)
        ...
        ->toHaveKey('{{namespace}}', 'AcmeLabs\\IconSetsHero')
        ->toHaveKey('{{namespaceEscaped}}', 'AcmeLabs\\\\IconSetsHero')
        ->toHaveKey('{{packageName}}', 'acme-labs/icon-sets-hero')
        ->toHaveKey('{{bladeNamespace}}', 'icon-sets-hero')
